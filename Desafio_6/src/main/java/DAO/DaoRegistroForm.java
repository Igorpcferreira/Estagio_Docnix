...
//idformulario ->1
    public boolean verificarDuplicidadeNoArrayDeDadosFormulario(List<RegistroDataForm> listaDeDados, Integer idRegistroFormulario) {
        for (int i = 0; i < listaDeDados.size(); i++) {
            if (listaDeDados.get(i).getIdFormulario().equals(idRegistroFormulario)) {
                return true;
            }
        }
        return false;
    ...
    public List<Contato> buscarSalvarNovaListaContatosAlternativos(List<Contato> listaDeContatos, Integer idFormulario){
        ...
        for(int i = 0; i<listaDeContatos.size(); i++){
            if(listaDeContatos.get(i).getId_formulario() == idFormulario){
                novaListaDeContatos.add(listaDeContatos.get(i));
            }
        }
    ...
    public List<RegistroDataForm> gravarFormulariosNaLista(EntityManager entityManager){
        ...
        for(int j=0; j< listaDeDadosFormulario.size();j++){
            List<Contato> novalistaDeContatos = buscarSalvarNovaListaContatosAlternativos(listaDeContatosAlternativos, listaDeDadosFormulario.get(j).getIdFormulario());
            listaDeDadosFormulario.get(j).setLista_de_contatos(novalistaDeContatos);
        }
