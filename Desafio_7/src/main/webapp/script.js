//Consumo de API
const URL_BASE = "http://localhost:8080/Desafio7"

const URL_LISTAR_REGISTRO_FORMULARIO = URL_BASE + "/RegistroFormulario/listar"

const URL_EXCLUIR_REGISTRO_FORMULARIO_BY_ID = URL_BASE + "/RegistroFormulario/excluir"

const URL_EDITAR_REGISTRO_FORMULARIO_BY_ID = URL_BASE + "/RegistroFormulario/editar"

const URL_INCLUIR_REGISTRO_FORMULARIO = URL_BASE + "/RegistroFormulario/incluir"
...
form.addEventListener("submit", function (e) {
    ...
    const formDataIncluir = new FormData(form);
    const formDataEditar =  new FormData(form)
    let formularioParaSerIncluido;
    let formularioParaSerEditado;


    async function enviarOuEditarFormularioParaOBackendEReconstruirATabela() {
        formularioParaSerIncluido = getOsDadosDoFormularioAPartirDoFormData(formDataIncluir)
        ...
        if (enviarOuEditar === 'editar') {
            ...
            for (let i = 0; i < registroFormularios.length; i++){
                if(idFormularioAserEditado === registroFormularios[i].idFormulario){
                    editarAux = i;
                }
            }
            console.log(editarAux)

            formularioParaSerEditado = getOsDadosDoFormularioParaSerEditadoAPartirDoFormData(formDataEditar, registroFormularios[editarAux]);
            console.log(JSON.stringify(formularioParaSerEditado))
            await editarRegistroFormulario(formularioParaSerEditado);
            enviarOuEditar = 'enviar';
        } else {
            await incluirRegistroFormulario(formularioParaSerIncluido, URL_INCLUIR_REGISTRO_FORMULARIO);
            console.log(JSON.stringify(formularioParaSerIncluido))
...
function getOsDadosDoFormularioAPartirDoFormData(formData) {
    ...
    for (let i = 0; i < quantidadeDeContatoss; i++) {
        objContatos[`nomeContato${i + 1}`]
            = formData.get(`contato-nome${i + 1}`)

        objContatos[`telefoneContato${i + 1}`]
            = formData.get(`contato-telefone${i + 1}`)

        objContatos[`emailContato${i + 1}`]
            = formData.get(`contato-email${i + 1}`)

        arrayContatos.push(objContatos)
    }

    formularioObjData.lista_de_contatos = arrayContatos
...
function getOsDadosDoFormularioParaSerEditadoAPartirDoFormData(formData, formulario) {
    ...
    formularioObjData.idFormulario = idFormularioAserEditado;

    let objContato = {}
    let arrayContatos = []

    for (let i = 0; i < quantidadeDeContatoss; i++) {
        objContato[`nomeContato${i + 1}`] = formData.get(`contato-nome${i + 1}`)
        objContato[`telefoneContato${i + 1}`] = formData.get(`contato-telefone${i + 1}`)
        objContato[`emailContato${i + 1}`] = formData.get(`contato-email${i + 1}`)
        arrayContatos[i] = objContato
        objContato = {}
    }

    formularioObjData.lista_de_contatos = arrayContatos;

    for(let i = 0; i < formularioObjData.lista_de_contatos.length; i++){
        formularioObjData.lista_de_contatos[i].idContato =
            formulario.lista_de_contatos[i].idContato
    }
